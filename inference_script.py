...
class CustomPredictor(nnUNetPredictor):
    def predict_single_npy_array(self, input_image: np.ndarray, priorChannels,image_properties: dict,
        ...
        with torch.no_grad():
            ...
            data, _ = preprocessor.run_case_npy(input_image, None, image_properties,
                                                ...
                                                self.dataset_json)
            # print(data.shape)
            # print(priorChannels.shape)
            data = np.concatenate([data, priorChannels], axis=0)
...
def predict_semseg(im, prop, semseg_trained_model, semseg_folds, priorChannels):
...
def map_labels_to_toothfairy(predicted_seg: np.ndarray) -> np.ndarray:
    ...
    remapping = {11: 21, 12: 22, 13: 23, 14: 24, 15: 25, 16: 26, 17: 27, 18: 28,
    19: 11, 20: 12, 21: 13, 22: 14, 23: 15, 24: 16, 25: 17, 26: 18,
    27: 41, 28: 42, 29: 43, 30: 44, 31: 45, 32: 46, 33: 47, 34: 48,
    35: 31, 36: 32, 37: 33, 38: 34, 39: 35, 40: 36, 41: 37, 42: 38}
    # Apply the remapping
...
def postprocess(prediction_npy, vol_per_voxel, verbose: bool = False):
    ...
    vol_per_voxel_cutoffs = 0.2 * 0.2 * 0.2
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--input_folder', type=Path, default="nnUnet_testRaw/images")
    parser.add_argument('-o', '--output_folder', type=Path, default="nnUnet_testRaw/AACN_pred")
    parser.add_argument('-sem_mod', '--semseg_trained_model', type=str,
                        default="nnUNet_results_AACN_26/Dataset119_ToothFairy2_All/nnUNetTrainer_onlyMirror01_500ep__nnUNetResEncUNetLPlans_torchres__3d_fullres")
    ...
    input_files = list(args.input_folder.glob('*.nii.gz')) + list(args.input_folder.glob('*.dcm'))

    for input_fname in input_files:
        print(input_fname.name)
        ...
        if os.path.exists(output_fname):
            ...
            with torch.no_grad():
                ###NC
                # print(type(im))
                # prob_map  = np.load("/mnt/public/home/shiliu_093424/code/yb/Test/output_logits_rename_resample/"+input_fname.name[:-7]+".npy")
                # sdf  = np.load("/mnt/public/home/shiliu_093424/code/yb/Test/output_sdf_map_rename_resample/"+input_fname.name[:-7]+"_sdf.npy")
                ####toothfairy2
                # prob_map  = np.load("/mnt/public/home/shiliu_093424/code/yb/nnUNet-master/nnUNet_raw_48/Dataset119_ToothFairy2_All/output_logits_resample/"+input_fname.name[:16]+"_0000.npy")
                # sdf  = np.load("/mnt/public/home/shiliu_093424/code/yb/nnUNet-master/nnUNet_raw_48/Dataset119_ToothFairy2_All/output_sdf_map_resample/"+input_fname.name[:16]+"_0000_sdf.npy")
                ##########一附院
                prob_map  = np.load("nnUnet_testRaw/aacn/output_logits/"+input_fname.name[:16]+"_0000.npy")
                sdf  = np.load("nnUnet_testRaw/aacn/output_sdf_map/"+input_fname.name[:16]+"_0000_sdf.npy")
                # print(prob_map.shape)
                # print(sdf.shape)
                sdf = np.expand_dims(sdf, axis=0)
                priorChannels = np.concatenate([prob_map[1:], sdf], axis=0)
                # print(im.shape)
                # print(priorChannels.shape)
                # im = np.concatenate([im, priorChannels], axis=0)
                semseg_pred = predict_semseg(im, prop, semseg_trained_model, semseg_folds, priorChannels)
                torch.cuda.empty_cache()
                gc.collect()
